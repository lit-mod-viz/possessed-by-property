...
with open(path + 'reports/07-actor-counts.csv', 'r') as csvfile:
    srlreader = csv.reader(csvfile, delimiter=',')

    for row in srlreader:
        actor_array.append(row)

with open(path + 'reports/07-benefactor-counts.csv', 'r') as csvfile:
    srlreader = csv.reader(csvfile, delimiter=',')
...
with open(path + 'reports/08-volitional-debt-ratios.csv', 'w') as write_file:
